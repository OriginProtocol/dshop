...
const Checkout = () => {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const tx = await chico.connect(signer).burn(ethers.utils.parseEther('1'))
  ...
  return (
    <>
      ...
      <div className="container flex mt-8 gap-6 items-start">
        <form onSubmit={onSubmit} style={{ flex: 6 }}>
          <div className="font-medium text-lg mb-4">
            Where should we send it?
          </div>
          <div className="grid grid-cols-1 gap-4 mb-6 relative bg-gray-900 bg-rounded-lg p-4 pb-6">
            ...
            <div>
              <CountrySelect
                className="w-full border-b px-2 py-3 bg-gray-900 placeholder-gray-400"
                value={state.country}
                onChange={(e) => {
            ...
            <div
              className={`grid grid-cols-${
                !country.provinces ? '1' : '2'
              } gap-2`}
            >
              {!country.provinces ? null : (
                <div>
                  <ProvinceSelect
                    className="border px-3 py-2 bg-gray-900 w-full"
                    country={country}
                    {...input('province')}
                  />
        ...
        <div className="ml-6" style={{ flex: 4 }}>
          <RedeemSummary item={item} />
        </div>
      </div>
    </>
  )
...
function initialState(cart) {
  return {
    ...
    className: 'w-full border-b px-2 py-3 bg-gray-900 placeholder-gray-400',
    defaultClassName: 'border-gray-400',
