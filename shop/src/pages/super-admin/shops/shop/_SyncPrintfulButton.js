...
const AdminSyncPrintful = ({ shop, className = '' }) => {
  ...
  useEffect(() => {
    ...
    fetch(`${config.backend}/shops/${shop.authToken}/sync-printful`, {
      headers: {
    ...
    <>
      ...
      {!state.sync ? null : (
        <Modal
          ...
          onClose={() => setState({ shouldClose: false, sync: false })}
        >
...
export default AdminSyncPrintful
